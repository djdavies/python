			...
			if event.type == pygame.QUIT:
				...
				if event.key == pygame.K_c:
				  # if key pressed is 'C' key, move CABBAGE (check if moving from left to right or other way)
					if cabbageMove == STOP and cabbagePosition.left > 300:
						cabbageMove = LEFT
					elif cabbageMove == STOP and cabbagePosition.left < 300:
						cabbageMove = RIGHT
							
				if event.key == pygame.K_w:
				  # if key pressed is 'W' key, move WOLF (check if moving from left to right or other way)
					if wolfMove == STOP and wolfPosition.left > 300:
						wolfMove = LEFT
					elif wolfMove == STOP and wolfPosition.left < 300:
						wolfMove = RIGHT
						
				if event.key == pygame.K_s:
				  # if key pressed is 'S' key, move SHEEP (check if moving from left to right or other way)
					if sheepMove == STOP and sheepPosition.left > 300:
						sheepMove = LEFT
					elif sheepMove == STOP and sheepPosition.left < 300:
						sheepMove = RIGHT
...
def winLoseCheck(farmerPosition,cabbagePosition,sheepPosition,wolfPosition): # checks to see if your a loser	
